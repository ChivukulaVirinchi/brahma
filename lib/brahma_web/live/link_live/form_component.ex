defmodule BrahmaWeb.LinkLive.FormComponent do
  ...
  def update(%{link: link} = assigns, socket) do
    changeset = Feed.change_link(link)
  ...
  defp save_link(socket, :edit, link_params) do
    case Feed.update_link(socket.assigns.link, link_params) do
      {:ok, link} ->
  ...
  defp save_link(socket, :new, link_params) do
    case Feed.create_link(socket.assigns.current_user, link_params) do
      {:ok, link} ->
        notify_parent({:saved, link})
